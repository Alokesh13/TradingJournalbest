...
function Pupil({ size=12, maxDistance=5, pupilColor="black", forceLookX=undefined, forceLookY=undefined }) {
...
function EyeBall({ size=48, pupilSize=16, maxDistance=10, eyeColor="white", pupilColor="black", isBlinking=false, forceLookX=undefined, forceLookY=undefined }) {
  const [mx,setMx] = useState(0); const [my,setMy] = useState(0);
  const ref = useRef(null);
  useEffect(() => {
    const h = (e) => { setMx(e.clientX); setMy(e.clientY); };
...
function AnimatedAuthPage({ onLogin, onBack }: { onLogin: (email: string) => void; onBack: () => void }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "signup") {
      ...
      accounts[email] = {
        email, username: username.trim(), passHash: hashPass(password),
        trades: [], darkMode: true, profilePicture: "", preferredAssets: ["XAUUSD"]
      };
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-[#080b14]">
      ...
      <div className="relative hidden lg:flex flex-col justify-between p-12 overflow-hidden"
        ...
        <div className="absolute bottom-1/3 right-1/4 w-64 h-64 rounded-full opacity-10 blur-3xl pointer-events-none"
        ...
        <div className="relative z-20 flex items-center justify-between">
          ...
          <button onClick={onBack}
            className="flex items-center gap-2 px-3 py-2 rounded-xl text-white/40 hover:text-white/80 border border-white/10 hover:border-white/20 transition-all duration-200 text-sm group"
        ...
        <div className="relative z-20 flex items-end justify-center" style={{ height: 420 }}>
          <div className="relative" style={{ width: 520, height: 400 }}>
            ...
            <div ref={purpleRef} className="absolute bottom-0 transition-all duration-700 ease-in-out"
              style={{
                left: 60, width: 170, height: lookingAway ? 440 : 400,
              ...
              <div className="absolute flex gap-8 transition-all duration-700 ease-in-out"
                ...
                <EyeBall size={18} pupilSize={7} maxDistance={5} eyeColor="white" pupilColor="#1a0533"
                  isBlinking={isPurpleBlinking}
                  forceLookX={lookingAway ? undefined : isLookingAtEachOther ? 3 : undefined}
                  forceLookY={lookingAway ? undefined : isLookingAtEachOther ? 4 : undefined} />
                <EyeBall size={18} pupilSize={7} maxDistance={5} eyeColor="white" pupilColor="#1a0533"
                  isBlinking={isPurpleBlinking}
                  forceLookX={lookingAway ? undefined : isLookingAtEachOther ? 3 : undefined}
                  forceLookY={lookingAway ? undefined : isLookingAtEachOther ? 4 : undefined} />
            ...
            <div ref={blackRef} className="absolute bottom-0 transition-all duration-700 ease-in-out"
              style={{
                left: 220, width: 115, height: 305,
                backgroundColor: "#1e2030", borderRadius: "8px 8px 0 0", zIndex: 2,
                transform: `skewX(${isLookingAtEachOther ? (bp.bodySkew || 0) * 1.5 + 10 : bp.bodySkew || 0}deg)`,
                transformOrigin: "bottom center"
              }}>
              <div className="absolute flex gap-6 transition-all duration-700 ease-in-out"
                style={{
                  left: isLookingAtEachOther ? 32 : 26 + (bp.faceX || 0),
                  top: isLookingAtEachOther ? 12 : 32 + (bp.faceY || 0)
                }}>
                <EyeBall size={16} pupilSize={6} maxDistance={4} eyeColor="white" pupilColor="#1e2030"
            ...
            <div ref={orangeRef} className="absolute bottom-0 transition-all duration-700 ease-in-out"
              style={{
                left: 0, width: 230, height: 195, zIndex: 3,
                backgroundColor: "#f97316", borderRadius: "115px 115px 0 0",
            ...
            <div ref={yellowRef} className="absolute bottom-0 transition-all duration-700 ease-in-out"
              style={{
                left: 295, width: 135, height: 225,
                backgroundColor: "#eab308", borderRadius: "68px 68px 0 0", zIndex: 4,
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-white/40 text-sm mt-1">
              {mode === "login" ? "Enter your credentials to access your journal." : "Start your private trading journal today."}
            </p>
          </div>
          ...
          <div className="mt-6 p-4 rounded-xl border border-white/5 text-xs text-white/30"
...
function InlineEdit({ value, onSave, type="text", options=null, dark=true }) {
  ...
  useEffect(() => { if (editing && inputRef.current) (inputRef.current as HTMLElement).focus(); }, [editing]);
  ...
  if (!editing) return (
    <span onClick={() => setEditing(true)}
      className={`cursor-pointer px-2 py-0.5 rounded hover:bg-white/10 transition-colors min-w-[40px] inline-block ${!dark ? "hover:bg-gray-100 text-gray-900" : ""}`}>
...
export default function App() {
  ...
  const handleScreenshotUpload = (e: React.ChangeEvent<HTMLInputElement>, isForm = true) => {
    ...
    files.forEach(file => {
      ...
      reader.onload = (ev) => {
        ...
        if (isForm) {
          setFormScreenshots(prev => [...prev, dataUrl]);
    ...
    mutate(u => {
      ...
      if (idx >= 0) {
        ...
        if (field === "outcome") {
          ...
          if (value === "Loss") {
            u.trades[idx].pl = Math.abs(pl) < 0 ? pl.toString() : (-Math.abs(pl)).toString();
            u.trades[idx].roi = (-Math.abs(roi)).toString();
          }
          if (value === "Win") {
            u.trades[idx].pl = Math.abs(pl).toString();
            u.trades[idx].roi = Math.abs(roi).toString();
          }
        }
  ...
  const exportData = () => {
    ...
    a.download = `tradetracker-${session}-${Date.now()}.json`;
    a.click();
  };
  ...
  if (session && !currentUser) {
    return <div className="flex items-center justify-center min-h-screen text-white" style={{ background: "#070a10" }}>Loading…</div>;
  }

  if (!session) {
    return (
      <AnimatedAuthPage
        onLogin={(email) => { handleLogin(email); setShowLanding(false); }}
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: bg, fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <aside className="flex-shrink-0 flex flex-col transition-all duration-300 relative z-40 h-screen overflow-y-auto"
        ...
        {/* Profile */}
        <div className={`flex ${sidebarOpen ? "items-center gap-3" : "flex-col items-center"} px-3 py-4 border-b`}
          ...
          <div className="relative flex-shrink-0">
            ...
            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2"
              style={{ background: "#10b981", borderColor: sidebarBg }} />
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => setPage(item.id)}
                ...
                style={{
                  background: active ? (dm ? "#1a1040" : "#ede9fe") : "",
                  color: active ? "#a78bfa" : (dm ? "#6b7280" : "#64748b")
                }}>
                <Icon size={18} />
        ...
        <div className="px-2 py-3 space-y-1 border-t" style={{ borderColor: sidebarBorder }}>
          <button onClick={toggleDark}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${sidebarOpen ? "" : "justify-center"}`}
            style={{ color: textSub }}>
          ...
          <label
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all cursor-pointer ${sidebarOpen ? "" : "justify-center"}`}
            style={{ color: textSub }}>
      ...
      {/* ── Main ── */}
      <main className="flex-1 h-screen overflow-y-auto" style={{ minWidth: 0, overscrollBehavior: "contain" }}>
        ...
        {page === "dashboard" && (
          <div className="p-6 space-y-6">
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      ...
                      <Tooltip formatter={(v, n) => [v, n]}
                        contentStyle={{ background: dm ? "#1a1d2e" : "#fff", border: `1px solid ${cardBorder}`, borderRadius: 12, fontSize: 12, color: textMain }} />
            ...
            {/* Statistics Chart */}
            <SectionCard title="Statistics" subtitle="P&L by time of day" dark={dm}
              extra={
                <div className="flex gap-1">
                  {(["days", "weeks", "months"] as const).map(v => (
                    <button key={v} onClick={() => setStatsView(v)}
                      ...
                      style={{
                        background: statsView === v ? (dm ? "#1a1040" : "#ede9fe") : "transparent",
                        color: statsView === v ? "#a78bfa" : textSub
                      }}>
                      {v}
                    </button>
                  ))}
                ...
                <table className="w-full text-xs min-w-[900px]">
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      return (
                        <tr key={t.id}
                          onClick={() => { setSelectedTrade(t); setDetailOpen(true); setEditMode(false); }}
                          ...
                          <td className="py-2 px-2">
                            <span className="px-2 py-0.5 rounded-full text-xs font-bold"
                              style={{
                                background: t.direction === "LONG" ? "#10b98122" : "#ef444422",
                          ...
                          <td className="py-2 px-2">
                            <span className="px-2 py-0.5 rounded-full text-xs font-bold"
                              style={{
                                background: t.outcome === "Win" ? "#10b98122" : t.outcome === "Loss" ? "#ef444422" : "#6b728022",
                                color: t.outcome === "Win" ? "#10b981" : t.outcome === "Loss" ? "#ef4444" : "#9ca3af"
                              }}>{t.outcome}</span>
                          </td>
                          <td className="py-2 px-2 font-mono" style={{ color: parseFloat(t.roi || "0") >= 0 ? "#10b981" : "#ef4444" }}>
                            {t.roi ? `${t.roi}%` : "—"}
                          </td>
                          ...
                          <td className="py-2 px-2">
                            <button
                              onClick={e => { e.stopPropagation(); if (confirm("Delete this trade?")) deleteTrade(t.id); }}
              ...
              <div className="rounded-2xl p-5 space-y-4" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={form.asset} onChange={e => setField("asset", e.target.value)} className={inputCls} style={inputStyle}>
                  ...
                  <div>
                    ...
                    <input value={form.pl} onChange={e => setField("pl", e.target.value)}
                      placeholder="e.g. 250 or -150" className={inputCls} style={inputStyle} />
                  </div>
                  <div>
                    <label className={labelCls} style={labelStyle}>ROI (%)</label>
                    <input value={form.roi} onChange={e => setField("roi", e.target.value)}
                      placeholder="e.g. 2.5" className={inputCls} style={inputStyle} />
                  </div>
                  <div>
                    <label className={labelCls} style={labelStyle}>Lot Size</label>
                    <input value={form.lotSize} onChange={e => setField("lotSize", e.target.value)}
                      placeholder="e.g. 0.10" className={inputCls} style={inputStyle} />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="datetime-local" value={form.startDate} onChange={e => setField("startDate", e.target.value)} className={inputCls} style={inputStyle} />
                  </div>
                  <div>
                    <label className={labelCls} style={labelStyle}>End Date & Time</label>
                    <input type="datetime-local" value={form.endDate} onChange={e => setField("endDate", e.target.value)} className={inputCls} style={inputStyle} />
                  </div>
                </div>
              </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatTile
                label="Profit Factor"
                value={losses > 0
                  ? (Math.abs(trades.filter(t => t.outcome === "Win").reduce((s, t) => s + parseFloat(t.pl || "0"), 0)) /
                    Math.abs(trades.filter(t => t.outcome === "Loss").reduce((s, t) => s + parseFloat(t.pl || "0"), 0))).toFixed(2)
                  : "∞"}
                sub="Gross profit / loss" icon={BarChart3} color="#7c3aed" dark={dm} />
              <StatTile
                label="Avg Win / Avg Loss"
                value={wins > 0 && losses > 0
                  ? `${(trades.filter(t => t.outcome === "Win").reduce((s, t) => s + parseFloat(t.pl || "0"), 0) / wins /
                    Math.abs(trades.filter(t => t.outcome === "Loss").reduce((s, t) => s + parseFloat(t.pl || "0"), 0) / losses)).toFixed(2)}x`
                  : "—"}
                sub="Risk/reward ratio" icon={Award} color="#f59e0b" dark={dm} />
            </div>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <ScatterChart margin={{ left: 0, right: 0, top: 5, bottom: 0 }}>
                  ...
                  <Tooltip cursor={{ strokeDasharray: "3 3" }}
                    contentStyle={{ background: dm ? "#1a1d2e" : "#fff", border: `1px solid ${cardBorder}`, borderRadius: 12, fontSize: 12, color: textMain }} />
                  <Scatter name="Trades"
                    data={trades.map(t => ({ roi: parseFloat(t.roi || "0"), pl: parseFloat(t.pl || "0"), outcome: t.outcome }))}
                    fill="#7c3aed">
            ...
            <div className="rounded-2xl p-5" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  const day = i + 1;
                  const k = day.toString();
                  const info = calDayMap[k];
                  const isToday = new Date().getDate() === day && new Date().getMonth() === calMonthIdx && new Date().getFullYear() === calYear;
                  return (
                    <div key={day}
                      className="rounded-xl p-2 min-h-[60px] flex flex-col items-center justify-start text-center transition-all"
                      style={{
                        background: info ? (info.pl >= 0 ? "#10b98115" : "#ef444415") : (isToday ? "#7c3aed15" : dm ? "#0a0d18" : "#f8fafc"),
          ...
          <div className="pointer-events-auto w-full max-w-md h-[calc(100vh-2rem)] flex flex-col rounded-2xl overflow-hidden shadow-2xl"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: dm ? "#1a1d2e" : "#e2e8f0" }}>
              ...
              <div className="flex gap-2">
                <button onClick={() => setEditMode(v => !v)}
                  ...
                  style={{ borderColor: editMode ? "#7c3aed" : dm ? "#1a1d2e" : "#e2e8f0", color: editMode ? "#7c3aed" : textSub }}>
                  <Edit2 size={15} />
                </button>
                <button onClick={() => { if (confirm("Delete?")) deleteTrade(selectedTrade.id); }}
                  className="p-2 rounded-xl border transition-colors"
                  style={{ borderColor: "#ef444433", color: "#ef4444" }}>
                  <Trash2 size={15} />
                </button>
                <button onClick={() => { setDetailOpen(false); setEditMode(false); }}
                  className="p-2 rounded-xl border transition-colors"
                  style={{ borderColor: dm ? "#1a1d2e" : "#e2e8f0", color: textSub }}>
                  <X size={15} />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              {[
                ...
                <div key={f.label} className="rounded-xl px-4 py-3"
                  style={{ background: dm ? "#0f1117" : "#f8fafc", border: `1px solid ${dm ? "#1e2130" : "#e2e8f0"}` }}>
                  ...
                  {editMode && f.field ? (
                    <InlineEdit value={(selectedTrade as any)[f.field]} dark={dm}
                      options={(f as any).opts}
                      onSave={v => updateTrade(selectedTrade.id, f.field!, v)} />
                  ) : (
                    <p className="font-semibold text-sm"
                      style={{
                        color: f.label === "P&L ($)"
                          ? (parseFloat(selectedTrade.pl || "0") >= 0 ? "#10b981" : "#ef4444")
                          : f.label === "ROI (%)"
                            ? (parseFloat(selectedTrade.roi || "0") >= 0 ? "#10b981" : "#ef4444")
                            : textMain
                      }}>
                      {f.value}
                    </p>
              ...
              <div className="rounded-xl px-4 py-3"
                ...
                <StarRating rating={selectedTrade.rating} size={18}
                  onRate={editMode ? r => updateTrade(selectedTrade.id, "rating", r) : null} />
              </div>
                      ...
                      <div key={i} className="relative group rounded-xl overflow-hidden aspect-video">
                        ...
                        {editMode && (
                          <button onClick={() => {
                            mutate(u => {
                              const idx = u.trades.findIndex(t => t.id === selectedTrade.id);
                              if (idx >= 0) { u.trades[idx].screenshots = u.trades[idx].screenshots.filter((_, j) => j !== i); }
                            });
